"""
@file: italian_posts_handlers.py
@description: Handlers для сценария "100 постов на итальянском"
@dependencies: aiogram, src.states
@created: 2026-01-30

Модуль содержит все handlers для сценария генерации 100 постов на итальянском.
Для использования вызови register_italian_handlers(bot_instance) из _register_handlers().
"""
...
from src.states import ItalianPostsStates


def register_italian_handlers(bot_instance):
    """
    Регистрация всех handlers для сценария "100 постов на итальянском".
    ...
    @dp.message(lambda m: m.text == "📹IT 100 post italiani")
    async def italian_posts_start_handler(message: types.Message, state: FSMContext):
        """Начало сценария 100 постов на итальянском"""
        await state.clear()
    
        if not is_allowed(message.from_user.id, "italian_posts"):
            await message.answer("❌ У вас нет доступа к этому сценарию")
            return

        info_text = """
    📹IT <b>100 постов на итальянском</b>

    Генерация уникальных постов о победах в слотах на итальянском языке.
    ...
    """
        ...
        await state.set_state(ItalianPostsStates.waiting_for_url1)

    @dp.message(ItalianPostsStates.waiting_for_url1, lambda m: m.text == "📖 Как именовать файлы")
    ...
    @dp.message(ItalianPostsStates.waiting_for_url1, lambda m: m.text == "🚀 Начать настройку")
    ...
    @dp.message(ItalianPostsStates.waiting_for_url1)
    async def streamer_posts_url1_handler(message: types.Message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.waiting_for_bonus1)
    ...
    @dp.message(ItalianPostsStates.waiting_for_bonus1)
    ...
    """
        ...
        await state.set_state(ItalianPostsStates.choosing_video_source)
        await message.answer(summary, reply_markup=keyboard, parse_mode="HTML")

    @dp.message(ItalianPostsStates.choosing_video_source, lambda m: m.text == "📡 Взять из канала")
    async def streamer_posts_choose_channel_source(message: types.Message, state: FSMContext):
        """Выбран источник - канал"""
        await state.set_state(ItalianPostsStates.waiting_for_source_channel)
    ...
    @dp.message(ItalianPostsStates.waiting_for_source_channel)
    async def streamer_posts_source_channel_handler(message: types.Message, state: FSMContext):
        ...
        if message.text == "🔗 Ссылка на пост":
            await state.set_state(ItalianPostsStates.waiting_for_post_link)
            await message.answer(
        ...
        try:
            ...
            await state.set_state(ItalianPostsStates.waiting_for_scan_direction)
    ...
    @dp.message(ItalianPostsStates.waiting_for_post_link)
    ...
    @dp.message(ItalianPostsStates.waiting_for_scan_direction)
    async def streamer_posts_direction_handler(message: types.Message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.waiting_for_video_range)

    @dp.message(ItalianPostsStates.waiting_for_video_range)
    ...
    async def _show_channel_video_for_metadata(message, state: FSMContext, index: int):
        ...
        await state.set_state(ItalianPostsStates.entering_metadata_for_channel)
    ...
    @dp.message(ItalianPostsStates.entering_metadata_for_channel)
    async def streamer_posts_channel_metadata_handler(message: types.Message, state: FSMContext):
        ...
        if 'currency' in video_data and video_data['currency']:
            ...
            currency = "EUR"  # По умолчанию EUR для итальянского сценария
    ...
    async def _proceed_to_target_channel(message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.waiting_for_target_channel)
    ...
    @dp.message(ItalianPostsStates.choosing_video_source, lambda m: m.text == "📤 Загрузить вручную")
    async def streamer_posts_choose_manual_upload(message: types.Message, state: FSMContext):
        """Выбрана ручная загрузка видео"""
        await state.set_state(ItalianPostsStates.waiting_for_videos)
    ...
    @dp.message(ItalianPostsStates.choosing_image_source, lambda m: m.text == "⏭ Без картинок")
    async def streamer_posts_skip_images(message: types.Message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.waiting_for_images)
    
        # Переходим к генерации
        await streamer_posts_images_done(message, state)

    @dp.message(ItalianPostsStates.choosing_image_source, lambda m: m.text == "📤 Загрузить картинки")
    ...
    @dp.message(ItalianPostsStates.choosing_image_source, lambda m: m.text == "📡 Картинки из канала")
    async def streamer_posts_images_from_channel(message: types.Message, state: FSMContext):
        """Картинки из канала"""
        await state.set_state(ItalianPostsStates.waiting_for_image_channel)
    ...
    @dp.message(ItalianPostsStates.waiting_for_image_channel)
    ...
    @dp.message(ItalianPostsStates.waiting_for_videos, lambda m: m.video is not None)
    async def streamer_posts_video_handler(message: types.Message, state: FSMContext):
        ...
        if parsed and parsed.bet > 0 and parsed.win > 0:
            ...
            await state.set_state(ItalianPostsStates.waiting_for_video_metadata)
    ...
    @dp.message(ItalianPostsStates.waiting_for_video_metadata)
    ...
    @dp.message(ItalianPostsStates.waiting_for_videos, lambda m: m.text == "✅ Видео готовы")
    ...
    @dp.message(ItalianPostsStates.waiting_for_images, lambda m: m.photo is not None)
    ...
    @dp.message(ItalianPostsStates.waiting_for_images, lambda m: m.text in ["✅ Картинки готовы", "⏭ Пропустить картинки"])
    async def streamer_posts_images_done(message: types.Message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.choosing_ai_model)
    ...
    @dp.callback_query(lambda c: c.data.startswith("ai_model:"), StateFilter(ItalianPostsStates.choosing_ai_model))
    async def streamer_posts_model_selected(callback: types.CallbackQuery, state: FSMContext):
        ...
        from src.ai_post_generator_it import AIPostGenerator, VideoData, OPENROUTER_MODELS
        ...
        def create_generator(m_key, m_provider):
            if m_provider == "openrouter":
                ...
                if model_info:
                    ...
                    # ИТАЛЬЯНСКИЙ СЦЕНАРИЙ: НЕ загружаем русские посты!
                    return gen
        ...
        # ИТАЛЬЯНСКИЙ СЦЕНАРИЙ: НЕ загружаем русские посты из my_posts.json!
        # AI будет генерировать чисто итальянский контент без влияния русских примеров
    ...
    """
        ...
        await state.set_state(ItalianPostsStates.waiting_for_uniqueness_check)
    ...
    """
        ...
        await state.set_state(ItalianPostsStates.preview_and_publish)
    ...
    @dp.message(ItalianPostsStates.waiting_for_uniqueness_check)
    async def streamer_posts_uniqueness_check_handler(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            if result.get("is_unique", True) and not duplicates:
                ...
                await state.set_state(ItalianPostsStates.showing_uniqueness_results)
    ...
    @dp.message(ItalianPostsStates.showing_uniqueness_results)
    ...
    @dp.message(ItalianPostsStates.preview_and_publish, lambda m: m.text == "👁 Ещё превью")
    ...
    @dp.message(ItalianPostsStates.waiting_for_target_channel, lambda m: m.text == "📋 Мои каналы")
    ...
    @dp.message(ItalianPostsStates.waiting_for_target_channel)
    async def streamer_posts_channel_handler(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            await state.set_state(ItalianPostsStates.choosing_image_source)
    ...
    @dp.message(ItalianPostsStates.confirming, lambda m: m.text == "🔄 Перегенерировать посты")
    @dp.message(ItalianPostsStates.preview_and_publish, lambda m: m.text == "🔄 Перегенерировать все")
    ...
    @dp.message(ItalianPostsStates.confirming, lambda m: m.text == "✅ Начать публикацию")
    @dp.message(ItalianPostsStates.preview_and_publish, lambda m: m.text == "✅ Начать публикацию")
    async def streamer_posts_start_publishing(message: types.Message, state: FSMContext):
        ...
        await state.set_state(ItalianPostsStates.processing)
    ...
    @dp.message(ItalianPostsStates.confirming, lambda m: m.text == "❌ Отмена")
    @dp.message(ItalianPostsStates.waiting_for_videos, lambda m: m.text == "❌ Отмена")
    @dp.message(ItalianPostsStates.waiting_for_images, lambda m: m.text == "❌ Отмена")
