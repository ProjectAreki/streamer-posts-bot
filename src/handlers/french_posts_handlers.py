"""
@file: french_posts_handlers.py
@description: Handlers для сценария "100 постов на французском"
...
from src.states import FrenchPostsStates


def register_french_handlers(bot_instance):
    """
    Регистрация всех handlers для сценария "100 постов на французском".
    
    Args:
        bot_instance: Экземпляр бота с доступом к dp, bot, config_manager и т.д.
    """
    ...
    @dp.message(lambda m: m.text == "📹FR 100 posts francais")
    async def french_posts_start_handler(message: types.Message, state: FSMContext):
        """Начало сценария 100 постов на французском"""
        await state.clear()
    
        if not is_allowed(message.from_user.id, "french_posts"):
            await message.answer("❌ У вас нет доступа к этому сценарию")
            return

        info_text = """
    📹FR <b>100 постов на французском</b>

    Генерация уникальных постов о победах в слотах на французском языке.
    ...
    """
        ...
        await state.set_state(FrenchPostsStates.waiting_for_url1)

    @dp.message(FrenchPostsStates.waiting_for_url1, lambda m: m.text == "📖 Как именовать файлы")
    ...
    @dp.message(FrenchPostsStates.waiting_for_url1, lambda m: m.text == "🚀 Начать настройку")
    ...
    @dp.message(FrenchPostsStates.waiting_for_url1)
    async def streamer_posts_url1_handler(message: types.Message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.waiting_for_bonus1)
    ...
    @dp.message(FrenchPostsStates.waiting_for_bonus1)
    ...
    """
        ...
        await state.set_state(FrenchPostsStates.choosing_video_source)
        await message.answer(summary, reply_markup=keyboard, parse_mode="HTML")

    @dp.message(FrenchPostsStates.choosing_video_source, lambda m: m.text == "📡 Взять из канала")
    async def streamer_posts_choose_channel_source(message: types.Message, state: FSMContext):
        """Выбран источник - канал"""
        await state.set_state(FrenchPostsStates.waiting_for_source_channel)
    ...
    @dp.message(FrenchPostsStates.waiting_for_source_channel)
    async def streamer_posts_source_channel_handler(message: types.Message, state: FSMContext):
        ...
        if message.text == "🔗 Ссылка на пост":
            await state.set_state(FrenchPostsStates.waiting_for_post_link)
            await message.answer(
        ...
        try:
            ...
            await state.set_state(FrenchPostsStates.waiting_for_scan_direction)
    ...
    @dp.message(FrenchPostsStates.waiting_for_post_link)
    ...
    @dp.message(FrenchPostsStates.waiting_for_scan_direction)
    async def streamer_posts_direction_handler(message: types.Message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.waiting_for_video_range)

    @dp.message(FrenchPostsStates.waiting_for_video_range)
            ...
            async for msg in client.iter_messages(**iter_params):
                if msg.video:
                    ...
                    video_info = {
                        ...
                        'bet': parsed.bet,
                        'win': parsed.win,
                        'streamer': '',  # Во французском сценарии стримеров НЕТ
                        'multiplier': parsed.multiplier,
                        'currency': parsed.currency,  # Добавляем валюту
                        'auto_parsed': parsed.bet > 0 and parsed.win > 0  # Для французского сценария слот не обязателен
                    }
                
                    videos_found.append(video_info)
                
                    # Для французского сценария слот НЕ обязателен - достаточно ставки и выигрыша
    ...
    async def _show_channel_video_for_metadata(message, state: FSMContext, index: int):
        ...
        await state.set_state(FrenchPostsStates.entering_metadata_for_channel)
        ...
        await message.answer(
            ...
            f"<code>Ставка | Выигрыш</code>\n"
            f"<code>Слот | Ставка | Выигрыш</code>\n\n"
            f"Примеры: <code>300 | 644580</code>\n"
            f"<code>Rip City | 300 | 644580</code>",
            parse_mode="HTML",
            reply_markup=keyboard
        )

    @dp.message(FrenchPostsStates.entering_metadata_for_channel)
    async def streamer_posts_channel_metadata_handler(message: types.Message, state: FSMContext):
        ...
        if len(parts) < 2:
            await message.answer(
                "❌ Неверный формат! Минимум 2 значения:\n"
                "<code>Ставка | Выигрыш</code>\n"
                "или со слотом:\n"
        ...
        video_data['streamer'] = ''  # Во французском сценарии стримеров НЕТ
        ...
        if 'currency' in video_data and video_data['currency']:
            ...
            currency = "EUR"  # По умолчанию EUR для французского сценария
    ...
    async def _proceed_to_target_channel(message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.waiting_for_target_channel)
    ...
    @dp.message(FrenchPostsStates.choosing_video_source, lambda m: m.text == "📤 Загрузить вручную")
    async def streamer_posts_choose_manual_upload(message: types.Message, state: FSMContext):
        """Выбрана ручная загрузка видео"""
        await state.set_state(FrenchPostsStates.waiting_for_videos)
        ...
        await message.answer(
            ...
            "Формат имени файла:\n"
            "<code>Ставка_Выигрыш.mp4</code>\n"
            "<code>Слот_Ставка_Выигрыш.mp4</code>\n\n"
    ...
    @dp.message(FrenchPostsStates.choosing_image_source, lambda m: m.text == "⏭ Без картинок")
    async def streamer_posts_skip_images(message: types.Message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.waiting_for_images)
    
        # Переходим к генерации
        await streamer_posts_images_done(message, state)

    @dp.message(FrenchPostsStates.choosing_image_source, lambda m: m.text == "📤 Загрузить картинки")
    ...
    @dp.message(FrenchPostsStates.choosing_image_source, lambda m: m.text == "📡 Картинки из канала")
    async def streamer_posts_images_from_channel(message: types.Message, state: FSMContext):
        """Картинки из канала"""
        await state.set_state(FrenchPostsStates.waiting_for_image_channel)
    ...
    @dp.message(FrenchPostsStates.waiting_for_image_channel)
    ...
    @dp.message(FrenchPostsStates.waiting_for_videos, lambda m: m.video is not None)
    async def streamer_posts_video_handler(message: types.Message, state: FSMContext):
        ...
        if parsed and parsed.bet > 0 and parsed.win > 0:
            ...
            video_info['streamer'] = ''  # Во французском сценарии стримеров НЕТ
            ...
            await state.set_state(FrenchPostsStates.waiting_for_video_metadata)
            ...
            await message.answer(
                ...
                "<code>Слот | Ставка | Выигрыш</code>\n\n"
                "Примеры:\n"
                "<code>500 | 125000</code>\n"
                "<code>Gates of Olympus | 500 | 125000</code>",
                parse_mode="HTML"
            )

    @dp.message(FrenchPostsStates.waiting_for_video_metadata)
    async def streamer_posts_metadata_handler(message: types.Message, state: FSMContext):
        ...
        currency = pending_video.get('currency', 'EUR')  # По умолчанию EUR для итальянского сценария
        text_upper = message.text.upper()
        ...
        pending_video['streamer'] = ''  # Во французском сценарии стримеров НЕТ
    ...
    @dp.message(FrenchPostsStates.waiting_for_videos, lambda m: m.text == "✅ Видео готовы")
    ...
    @dp.message(FrenchPostsStates.waiting_for_images, lambda m: m.photo is not None)
    ...
    @dp.message(FrenchPostsStates.waiting_for_images, lambda m: m.text in ["✅ Картинки готовы", "⏭ Пропустить картинки"])
    async def streamer_posts_images_done(message: types.Message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.choosing_ai_model)
    ...
    @dp.callback_query(lambda c: c.data.startswith("ai_model:"), StateFilter(FrenchPostsStates.choosing_ai_model))
    async def streamer_posts_model_selected(callback: types.CallbackQuery, state: FSMContext):
        ...
        from src.ai_post_generator_fr import AIPostGenerator, VideoData, OPENROUTER_MODELS
        ...
        def create_generator(m_key, m_provider):
            if m_provider == "openrouter":
                ...
                if model_info:
                    ...
                    # ФРАНЦУЗСКИЙ СЦЕНАРИЙ: НЕ загружаем русские посты!
                    return gen
        ...
        # ФРАНЦУЗСКИЙ СЦЕНАРИЙ: НЕ загружаем русские посты из my_posts.json!
        # AI будет генерировать чисто французский контент без влияния русских примеров
        ...
        if is_rotation:
            ...
            # КРИТИЧНО: генерируем AI-пул описаний бонусов ОДИН РАЗ перед ротацией
            bonus1_pool = []
            try:
                await status_msg.edit_text(
                    "🎯 <b>Генерация уникальных описаний бонусов...</b>\n\n"
                    f"📝 Создаём {total_posts} уникальных описаний для ссылки",
                    parse_mode="HTML"
                )
                pool_gen = create_generator("gemini-3-flash", "openrouter")
                pool_gen.set_bonus_data(
                    url1=data['url1'],
                    bonus1=data['bonus1']
                )
                await pool_gen.generate_bonus_descriptions_pool(count=total_posts)
                bonus1_pool, = pool_gen.get_bonus_pool()
                logger.info(f"AI-пул описаний бонусов: {len(bonus1_pool)}")
            ...
            for i, video in enumerate(video_data_list):
                ...
                if bonus1_pool:
                    rot_generator.set_bonus_pool(bonus1_pool)
                ...
                try:
                    ...
                    try:
                        ...
                        if bonus1_pool:
                            fallback_gen.set_bonus_pool(bonus1_pool)
            ...
            if images:
                ...
                if bonus1_pool:
                    img_generator.set_bonus_pool(bonus1_pool)
            ...
            await generator.generate_bonus_descriptions_pool(80)
    ...
    """
        ...
        await state.set_state(FrenchPostsStates.waiting_for_uniqueness_check)
    ...
    """
        ...
        await state.set_state(FrenchPostsStates.preview_and_publish)
    ...
    @dp.message(FrenchPostsStates.waiting_for_uniqueness_check)
    async def streamer_posts_uniqueness_check_handler(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            if result.get("is_unique", True) and not duplicates:
                ...
                await state.set_state(FrenchPostsStates.showing_uniqueness_results)
    ...
    @dp.message(FrenchPostsStates.showing_uniqueness_results)
        ...
        if "перегенерировать дубли" in text:
            ...
            try:
                ...
                try:
                    ...
                    pass
                generator.set_bonus_data(url1=url1, bonus1=bonus1)
                ...
                for idx in posts_to_regenerate:
                    ...
                    if not video_info:
                        video_info = {
                            ...
                            'bet': 100, 'win': 10000, 'currency': 'EUR'
                        }
                    ...
                    video_data = VideoData(
                        ...
                        currency=video_info.get('currency', 'EUR')
                    )
    ...
    @dp.message(FrenchPostsStates.preview_and_publish, lambda m: m.text == "👁 Ещё превью")
    ...
    @dp.message(FrenchPostsStates.waiting_for_target_channel, lambda m: m.text == "📋 Мои каналы")
    ...
    @dp.message(FrenchPostsStates.waiting_for_target_channel)
    async def streamer_posts_channel_handler(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            await state.set_state(FrenchPostsStates.choosing_image_source)
    ...
    @dp.message(FrenchPostsStates.confirming, lambda m: m.text == "🔄 Перегенерировать посты")
    @dp.message(FrenchPostsStates.preview_and_publish, lambda m: m.text == "🔄 Перегенерировать все")
    async def streamer_posts_regenerate(message: types.Message, state: FSMContext):
        ...
        from src.ai_post_generator_fr import AIPostGenerator, VideoData
    ...
    @dp.message(FrenchPostsStates.confirming, lambda m: m.text == "✅ Начать публикацию")
    @dp.message(FrenchPostsStates.preview_and_publish, lambda m: m.text == "✅ Начать публикацию")
    async def streamer_posts_start_publishing(message: types.Message, state: FSMContext):
        ...
        await state.set_state(FrenchPostsStates.processing)
    ...
    @dp.message(FrenchPostsStates.confirming, lambda m: m.text == "❌ Отмена")
    @dp.message(FrenchPostsStates.waiting_for_videos, lambda m: m.text == "❌ Отмена")
    @dp.message(FrenchPostsStates.waiting_for_images, lambda m: m.text == "❌ Отмена")
