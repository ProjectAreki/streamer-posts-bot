...
class AIPostGenerator:
    ...
    SYSTEM_PROMPT_ARCHITECT = """Вы — архитектор вирусного контента для Telegram.
...
📋 ВЫБЕРИ ОДИН из 9 форматов ниже (меняй каждый раз!):

1️⃣ ЭМОДЗИ + URL + ТИРЕ + ОПИСАНИЕ:
...
3️⃣ СТРЕЛКА + URL + НОВАЯ СТРОКА + ОПИСАНИЕ:
   ➡️ {url1}
...
9️⃣ ТИРЕ + URL + ТИРЕ + ОПИСАНИЕ:
   - {url1} — до 100.000 рублей на счет и 100 бесплатных спинов
   - {url2} — бонус до 50.000 рублей и фриспины в подарок

📏 ДЛИНА: 600-900 символов (Telegram limit = 1024)"""

    # ═══════════════════════════════════════════════════════════════════
    # СИСТЕМНЫЙ ПРОМПТ 3 (для ротации - будет заменён пользователем)
    # ═══════════════════════════════════════════════════════════════════
    
    SYSTEM_PROMPT_3 = """🎯 ТВОЯ РОЛЬ: Ты — гуру вовлекающих текстов для Telegram. Твоя сверхзадача — превращать каждый пост в маленькое событие, от которого невозможно оторваться.
...
⚠️ КРИТИЧЕСКИ ВАЖНО: ЕСЛИ УКАЗАН НИК СТРИМЕРА ({streamer}) — ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙ ЕГО МИНИМУМ 3-4 РАЗА!
• "<b>{streamer}</b> показал результат!" — упоминай в заголовке
• "Наш <b>{streamer}</b> не подвёл!" — упоминай в середине
• "Повторить успех <b>{streamer}</b>" — упоминай в конце/CTA
...
📏 ДЛИНА: 600-900 символов (Telegram limit = 1024)"""
    ...
    SYSTEM_PROMPT_4 = """👋 ПРИВЕТ, ГЕНИЙ КОНТЕНТА! Ты создаешь не посты, а вирусные эмоции для Telegram. Каждое твое сообщение должно хватать за живое и не отпускать до последнего символа.
...
📏 ДЛИНА: 600-900 символов (Telegram limit = 1024)"""
    ...
    SYSTEM_PROMPT_5 = """Вы — архитектор вирусного контента. Ваша задача — проектировать не просто посты, а самоподдерживающиеся механики вовлечения для Telegram-аудитории. Каждый элемент текста должен работать на удержание внимания и целевое действие.
...
📏 ДЛИНА: 600-900 символов (Telegram limit = 1024)"""
    ...
    SYSTEM_PROMPT_6 = """ЗАДАНИЕ: Создавай уникальный, живой контент для TG. Каждый пост — новая форма и подход.
...
📏 ДЛИНА: 600-900 символов (Telegram limit = 1024)"""
    ...
    SYSTEM_PROMPT = """Ты — копирайтер для Telegram-канала про стримеров.
...
• ИМЯ СТРИМЕРА — это РЕАЛЬНЫЙ НИК, используй его МИНИМУМ 3-4 РАЗА в посте!
• "<b>{streamer}</b> сегодня в ударе!" — упоминай в заголовке
• "Наш <b>{streamer}</b> не подвёл!" — упоминай в середине
• "Повторить успех <b>{streamer}</b>" — упоминай в конце/CTA
• Строй ВЕСЬ пост вокруг стримера, он — звезда истории!
• НИКОГДА не игнорируй ник стримера, если он указан в данных!
...
ДАННЫЕ:
• Стример: {streamer} (если указан — используй минимум 3 раза!)
• Слот: {slot}
• Ставка: {bet}{currency}
...
Стиль: расскажи про бонусы как другу.
...
Стиль: объясни почему стоит попробовать.
...
- Гиперссылки HTML: <a href="url">текст</a>""",

        """Напиши пост-СОВЕТ про бонусы.
Ссылки: {url1} ({bonus1}), {url2} ({bonus2}).

Стиль: как лайфхак от друга.
...
Стиль: создай ажиотаж!
ПОЗИЦИЯ ССЫЛОК: в КОНЦЕ поста.
...
Стиль: как будто уже попробовал.
...
до 100.000 рублей на счет и плюсом 100 бесплатных спинов!""",
    ...
    def _get_system_prompt(self) -> str:
        ...
        Использует более случайную ротацию, чтобы избежать предсказуемости.
        """
        ...
        # Каждые 6 постов используем строгую ротацию, между ними - случайную
        # Это дает баланс между разнообразием и предсказуемостью
            ...
            try:
                ...
                if has_real_streamer and streamer_name:
                    ...
                    if streamer_mentions < 2:
                        print(f"   ⚠️ Ник '{streamer_name}' найден {streamer_mentions} раз(а), нужно >= 2. Регенерируем...")
                        sys.stdout.flush()
                        continue
    ...
    async def generate_image_post(self, index: int = 0) -> GeneratedPostAI:
        ...
        for regen in range(1, max_regens + 1):
            try:
                ...
                # 5. Умное сокращение если слишком длинный (сохраняем ссылки!)
                if len(text) > 1020:
                    print(f"   ✂️ Image пост слишком длинный ({len(text)} символов), сокращаем...")
                    text = self._smart_trim_text(text, 1000)
                    print(f"   ✅ После сокращения: {len(text)} символов")
