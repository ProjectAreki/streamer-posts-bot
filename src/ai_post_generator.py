...
до 100.000 рублей на счет и плюсом 100 бесплатных спинов!""",
    ...
    async def _request_bonus_pool(self, original_desc: str, count: int, is_bonus1: bool) -> List[str]:
        ...
        prompt = f"""Сгенерируй {request_count} УНИКАЛЬНЫХ описаний бонуса для Telegram-постов.

БОНУС: "{original_desc}"

КОНТЕКСТ: Это описание стоит рядом со ссылкой в посте. Читатель должен захотеть перейти и забрать этот бонус. Пиши как живой человек — друг, который делится находкой. Каждое описание — самостоятельное, не зависит от других ссылок.

ЦИФРЫ ПИШИ ЦИФРАМИ: 100 000, 100к, 150%, 100 спинов. НЕ словами!

ЧТО ДОЛЖНО БЫТЬ В КАЖДОМ ОПИСАНИИ:
- Точные цифры из оригинала (суммы, проценты, количество спинов)
- Мотивация перейти по ссылке и забрать бонус
- Ощущение что это выгодная возможность которую жалко упустить

ЗАПРЕЩЕНО:
- Сухое перечисление цифр через плюс без подводки
- Сравнение с другими бонусами/ссылками ("а если хочешь больше", "для серьёзных")
- Повторение одних и тех же слов и конструкций из описания в описание
- HTML-теги, эмодзи

КРИТИЧНО — РАЗНООБРАЗИЕ:
- Каждое описание 10-30 слов
- Все {request_count} штук должны быть РАЗНЫМИ по структуре, началу, стилю и подаче
- НЕ используй одни и те же вводные слова и обороты повторно
- Чередуй тон: разговорный, уверенный, дружеский, деловой, интригующий, расслабленный
- Валюту сохрани: рубли=₽/руб/тыс/к, доллары=$

ФОРМАТ: JSON массив строк. ТОЛЬКО JSON, без комментариев."""
