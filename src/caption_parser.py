...
class CaptionParser:
    ...
    PATTERNS = {
        ...
        # Выигрыш (рРpP — кирилл. и латин. R/P как сокращение рублей, напр. "P2.942.485")
        'win': [
            r'выигрыш[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # выигрыш P2.942.485 / выигрыш 644580.00
            r'ganancia[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Ganancia: 498.095$ (испанский)
            r'[Vv]incita[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Vincita: 505 € (итальянский)
            r'[Gg]agner[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Gagner: 360 € (французский)
            r'[Gg]ain[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Gain: 360 € (французский)
            r'💰\s*[$₽€£рРpP\s]*([\d\s,.]+)',  # 💰 $ 89 000
            r'получил[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # получил $ 125000
            r'забрал[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # забрал $ 125000
            r'вин[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # вин: $ 125000
            r'win[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # win: $ 125000
            r'→\s*[$₽€£рРpP\s]*([\d\s,.]+)',  # → $ 125000
        ],
        # Ставка (рРpP — кирилл. и латин. R/P как сокращение рублей)
        'bet': [
            r'[сc]тавка[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # ставка P500 / ставка 1 USD
            r'apuesta[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Apuesta: 100$ (испанский)
            r'[Pp]untata[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Puntata: 50 € (итальянский)
            r'[Mm]ise[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # Mise: 10 € (французский)
            r'💵\s*[$₽€£рРpP\s]*([\d\s,.]+)',  # 💵 $ 200
            r'вход[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # вход: $ 500
            r'бет[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # бет: $ 500
            r'bet[:\s]*[$₽€£рРpP\s]*([\d\s,.]+)',  # bet: $ 500
        ],
        # Стример (опционально)
        'streamer': [
            r'стример[:\s]+([^\n\r|]+)',  # стример: Жека
    ...
    def parse(cls, caption: str) -> ParsedCaption:
        ...
        for pattern in cls.PATTERNS['slot']:
            ...
            if match:
                ...
                # Убираем валюту+цифры с конца (₽, русская р рядом с числами)
                # Только если есть символ валюты — голые цифры НЕ трогаем (Sugar Rush 1000)
                result.slot = re.sub(r'\s+[\d\s,.]*[₽р]+[\d\s,.₽р]*$', '', result.slot).strip()
                result.slot = re.sub(r'\s*[₽]+$', '', result.slot).strip()
                break
        ...
        # Защита: выигрыш не может быть меньше ставки в постах про выигрыши
        # Если win < bet — скорее всего данные перепутаны местами, меняем
        if result.bet > 0 and result.win > 0 and result.win < result.bet:
            result.win, result.bet = result.bet, result.win
    ...
    def _parse_number(s: str) -> float:
        ...
        # Пример: 1.234.567,89 → 1234567.89 (европейский с запятой-десятичной)
        if dot_count > 1:
            parts = s.split('.')
            last_group = parts[-1]
            if ',' in last_group:
                # Европейский формат с запятой-десятичной: 1.234.567,89
                s = s.replace('.', '')
                s = s.replace(',', '.')
                ...
                s = s.replace('.', '')
        # Случай 2: одна точка и одна или более запятых — определяем формат по позиции
        # Европейский: 492.590,00 (точка перед запятой → точка=тысячи, запятая=десятичная)
        # Американский: 1,234.56 (запятая перед точкой → запятая=тысячи, точка=десятичная)
        elif dot_count == 1 and comma_count > 0:
            dot_pos = s.index('.')
            comma_pos = s.index(',')
            if dot_pos < comma_pos:
                # Европейский: 492.590,00 → точка=тысячи, запятая=десятичная
                s = s.replace('.', '')
                s = s.replace(',', '.')
            else:
                # Американский: 1,234.56 → запятая=тысячи, точка=десятичная
                s = s.replace(',', '')
