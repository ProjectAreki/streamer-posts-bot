...
class CaptionParser:
    ...
    PATTERNS = {
        ...
        # Ставка
        'bet': [
            r'ставка[:\s]*[$₽€£\s]*([\d\s,.]+)',  # ставка 1 USD или ставка 300
            r'💵\s*[$₽€£\s]*([\d\s,.]+)',  # 💵 $ 200
    ...
    def _parse_number(s: str) -> float:
        ...
        # Случай 5: одна точка (может быть десятичным разделителем)
        elif dot_count == 1:
            parts = s.split('.')
            if len(parts) == 2 and len(parts[1]) <= 2:
                ...
                # Точка как разделитель тысяч - удаляем
                s = s.replace('.', '')
