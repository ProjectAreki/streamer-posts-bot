...
class CaptionParser:
    ...
    PATTERNS = {
        ...
        # Выигрыш (рР — кириллическая Р как сокращение рублей, напр. "Р2.942.485")
        'win': [
            r'выигрыш[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # выигрыш Р2.942.485 / выигрыш 644580.00
            r'ganancia[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Ganancia: 498.095$ (испанский)
            r'[Vv]incita[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Vincita: 505 € (итальянский)
            r'[Gg]agner[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Gagner: 360 € (французский)
            r'[Gg]ain[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Gain: 360 € (французский)
            r'💰\s*[$₽€£рР\s]*([\d\s,.]+)',  # 💰 $ 89 000
            r'получил[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # получил $ 125000
            r'забрал[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # забрал $ 125000
            r'вин[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # вин: $ 125000
            r'win[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # win: $ 125000
            r'→\s*[$₽€£рР\s]*([\d\s,.]+)',  # → $ 125000
        ],
        # Ставка (рР — кириллическая Р как сокращение рублей)
        'bet': [
            r'[сc]тавка[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # ставка Р500 / ставка 1 USD
            r'apuesta[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Apuesta: 100$ (испанский)
            r'[Pp]untata[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Puntata: 50 € (итальянский)
            r'[Mm]ise[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # Mise: 10 € (французский)
            r'💵\s*[$₽€£рР\s]*([\d\s,.]+)',  # 💵 $ 200
            r'вход[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # вход: $ 500
            r'бет[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # бет: $ 500
            r'bet[:\s]*[$₽€£рР\s]*([\d\s,.]+)',  # bet: $ 500
        ],
        # Стример (опционально)
        'streamer': [
            r'стример[:\s]+([^\n\r|]+)',  # стример: Жека
