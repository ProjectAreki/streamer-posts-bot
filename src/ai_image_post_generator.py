    ...
    async def generate_post(
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # Обрезаем до 600 символов если слишком длинный
                if len(text) > 600:
                    print(f"   ✂️ Пост #{index}: Обрезаю с {len(text)} до 600 символов")
                    text = self._smart_trim(text, 600)
                    # Проверяем что ссылки остались после обрезки
                    if self.url1 not in text or self.url2 not in text:
                        print(f"   ⚠️ Пост #{index}: Ссылки пропали после обрезки, попытка {attempt}/{max_attempts}")
                        continue
